...
**Nothing is hidden after it has been painted, and the hero never waits for JavaScript.** A reveal that arms an element on `mounted` makes the browser show it, then hide it, then fade it back in — the flash reads as a half-drawn component, and it is worst on buttons. Worse still, a hero that *starts* on `mounted` stays blank until hydration finishes, which on a heavy route reads as a frozen page. Two rules keep both from happening. The hero intro is **pure CSS** under `[data-hero="root"]` in `__animations.scss` — three `@keyframes` (`heroWordIn`, `heroRiseIn`, `heroMediaIn`) with `animation-fill-mode: backwards`, so every part holds its start state from frame one and the timeline runs off the inlined stylesheet at first paint, with no JS, no hydration wait, no failsafe needed. Per-part offsets are `nth-child` delay rules, so nothing has to be measured or stamped at runtime. `v-reveal` measures the element in a batched `requestAnimationFrame` and **skips arming anything already inside the viewport** — content the visitor can already see stays exactly as it was painted, and only what is still below the fold gets a reveal.
...
| `data-hero="title\|lead\|actions\|card\|media"` | Staged hero timeline, pure CSS keyframes, starts at first paint                                               | `PageHero`, `HomeHero`                                    |
| `HeroTitle`                     | Server-rendered masked word split for a hero `h1` — the one `data-hero="title"`                                               | `PageHero`, `HomeHero`                                    |
...
| Element                                      | Mechanism                                                                                                  |
| -------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| Home hero `h1`                               | `HeroTitle` masked words, CSS `heroWordIn`                                                                 |
...
Mixins: `bp.down($tier)`, `bp.up($tier)`, `bp.only($tier)`, `bp.between($from, $to)`, `bp.short` (`max-height: 500px`), `bp.reduced-motion`, `bp.touch` (`hover: none`) and `bp.mouse` (`hover: hover`) — pointer capability, not width. Import alias is always `bp`.
